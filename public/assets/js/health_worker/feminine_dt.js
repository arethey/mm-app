...
$(document).ready(function () {
    ...
    $('#viewFeminineModal').on('show.bs.modal', function (e) {
        ...
        if (last_period_dates.length != 0) {
            ...
            modal.find('.modal-body #view_last_periods').append('<p class="text-muted m-0">• No record found</p>');
        }
    });
    ...
    $('#editFeminineModal').on('show.bs.modal', function (e) {
        ...
        if (e.relatedTarget != null) {
            ...
            modal.find('.modal-body #edit_last_period_date').datepicker('setDate', button.data('last_period_date'));
            modal.find('.modal-body #edit_birthdate').datepicker('setDate', button.data('birthdate'));
            ...
            modal.find('.modal-body #edit_menstruation_status option').removeAttr("selected").end().find('#edit_menstruation_status option[value="' + button.data('menstruation_status') + '"]').attr("selected", true).end();
        }
    });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        iziToast.info({
                            ...
                            drag: false,
                            title: 'Success!',
                            message: 'Feminine has been successfully removed from the list.',
    ...
    $('#assignFeminineModal').on('show.bs.modal', function (e) {
        $.ajax({
            ...
            type: 'GET',
            data: {
                health_worker_id: $(document).find('health_worker_id').val(),
            },
            dataType: 'json',
