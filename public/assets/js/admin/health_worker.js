...
$(document).ready(function () {
    ...
    $.validator.setDefaults({
        submitHandler: function (content) {
            ...
            var form = action == 0 ? $('#newHealthWorkerForm') : $('#editHealthWorkerForm');
            var url = action == 0 ? '../admin/post-health-worker' : '../admin/update-health-worker';
            var modal = action == 0 ? $('#newHealthWorkerModal') : $('#editHealthWorkerModal');
    ...
    $('#editHealthWorkerModal').on('show.bs.modal', function (e) {
        ...
        if (e.relatedTarget != null) {
            ...
            modal.find('.modal-body #edit_birthdate_datepicker').datepicker('setDate', button.data('birthdate'));
    ...
    $('#viewHealthWorkerModal').on('show.bs.modal', function (e) {
        ...
        if (assigned_feminine_list.length != 0) {
            $.each(assigned_feminine_list, function (i, value) {
                var assigned = $('<p class="text-muted mt-' + (i == 0 ? '1' : '0') + ' mb-0 assigned_' + value.feminine_health_worker_group_id + '">• ' + value.full_name + '<i class="fa-solid fa-trash float-right text-danger btn-icon remove_assign" data-id="' + value.feminine_health_worker_group_id + '" role="button"></i></p>');
                modal.find('.modal-body #view_feminnine_assign').append(assigned);
            });
        }
        else {
            modal.find('.modal-body #view_feminnine_assign').append('<p class="text-muted m-0">• No record found</p>');
        }
    });
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (response) {
                        $(document).find('.assigned_'+assign_id).remove();
                        if (response.updated_count == 0) $(document).find('#view_feminnine_assign').append('<p class="text-muted m-0">• No record found</p>');
    ...
    $('#assignFeminineModal').on('show.bs.modal', function (e) {
        ...
        $(document).find('#display_health_worker_name').text($(button).data('health_worker_name'));
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.feminine_list, function (index, feminine) {
                    selectElement.append('<option value="' + feminine.id + '">' + feminine.full_name + '</option>');
                });
